...
### telephony_v3 模块导致网络异常 / 断网

`fix_k40s_telephony_v3` 内含守护进程持续覆盖 `gsm.network.type` / `gsm.version.baseband`，在 **MuMu 模拟器上会导致网络异常、APP 无法联网**。该模块已禁用，源码见 `fix_k40s_telephony_v3.disabled`。

> 如需重新启用，将 `.disabled` 改回 `.zip` 即可。**仅建议在真机上使用。**
