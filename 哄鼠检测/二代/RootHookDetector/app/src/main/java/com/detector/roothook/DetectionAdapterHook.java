...
public class DetectionAdapterHook extends RecyclerView.Adapter<DetectionAdapterHook.ViewHolder> {

    private final List<HookDetector.DetectionResult> results;

    public DetectionAdapterHook(List<HookDetector.DetectionResult> results) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        HookDetector.DetectionResult result = results.get(position);
        holder.tvName.setText(result.name);
        holder.tvDetail.setText(result.detail);
        if (result.detected) {
            holder.tvStatus.setText("⚠️ 已检测");
